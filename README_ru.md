...
## $fieldsModel
В $fieldsModel описывается модель данных в виде ассоциативного массива. Ключ - это имя свойства объекта, значение - массив с описанием параметров свойства.
...
| **timeUpdate**     | Можно применять только на типе `datetime`. Устанавливает текущее время при сохранении объекта методом [save()](#метод-save).                                                               |
...
| **afterSave**   | Вызывается после сохранения. В метод передается массив имен свойств, которые были отправлены в MongoDB. |
```PHP
class User extends ModelMongoDB
{
